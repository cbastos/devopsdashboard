...
<@layout.registrationLayout displayInfo=social.displayInfo; section>
    ...
    <#elseif section = "form">
        <div>
            <img class="logo" src="${url.resourcesPath}/img/logo.png" alt="metrics logo">
        </div>
        <div class="box-container">
            <div>
                <p class="application-name">Sign In</p>
            </div>
        <#if realm.password>
            <div>
               <form id="kc-form-login" class="form" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                ...
                <script>
                   ...
                   var loginAction = form.action;
                   form.action = loginAction.replace('http://localhost/', 'http://localhost:85/');
                </script>
            </div>
        </#if>
        <#if social.providers??>
